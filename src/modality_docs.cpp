...
// #include <bson.h>              // Converting from/to BSON - temporarily disabled
#define USTORE_DISABLE_BSON 1
// #include <mpack_header_only.h> // Converting from/to MsgPack - temporarily disabled
#define USTORE_DISABLE_MSGPACK 1
    ...
    if (!c.keys) {
        ...
        for (size_t idx = 0; idx < c.tasks_count; ++idx, ++vals, ++lens) {
            ...
            return_error_if_m(result.type() == simdjson::ondemand::json_type::number,
                              c.error,
                              args_wrong_k,
                              "The id field does not have the ustore key type");
            tape[idx] = result;
        }
...
#endif
#ifndef USTORE_DISABLE_BSON
        if (c.type == ustore_doc_field_bson_k) {
            bson_error_t error;
